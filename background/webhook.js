// background/webhook.js — ES module version av Webhook för service worker
// components/webhook.js är kvar som classic script för content scripts
...
export const Webhook = {
  async send(webhookUrl, content) {
    ...
    if (!cleanWebhookUrl) {
      ...
      return { success: false, error: 'empty_webhook' };
    }
    ...
    try {
      ...
      if (!response.ok) {
        let errorText = '';
        try { errorText = await response.text(); } catch { errorText = 'Kunde inte läsa response body'; }
        const message = `[Discord Sender] Webhook-fel ${response.status} ${response.statusText}: ${errorText}`;
        console.error(message);
        return { success: false, error: response.status, status: response.status, statusText: response.statusText, details: errorText };
      }
      ...
      const errorMessage = err?.message ?? 'Okänt nätverksfel';
      console.error('[Discord Sender] Nätverksfel:', errorMessage);
      return { success: false, error: 'network_error', details: errorMessage };
    }
  }
};
